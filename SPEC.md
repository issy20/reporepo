...
最終更新: 2026-08-11
バージョン: 0.1.0 (開発中)
ステータス: コア・TUI・CLI・CLIプレゼンテーション・OS資格情報ストア移行・コード解析・キャッシュ鮮度管理・analyzeコマンド・疑似Trending一覧・学習ノート実装完了 / 並行アクセス整合性は仕様確定・未実装 / OS別手動スモークテスト未完了
...
製品の主題である学習の記録として、各リポジトリのエントリに自分のメモを保存・編集・表示できる。AI 解析結果は「参照用」で、ユーザーが書き足すノートが学習の蓄積としての価値を持つ。

**データモデル。** `Entry.Note string`（`json:"note"`）を追加する。ノートはリポジトリ単位（言語別ではない）。既存 `data.json` には `note` がなくゼロ値（空文字）として扱い、マイグレーション不要。

**TUI（詳細画面）。** 詳細画面の解析結果の下にノートを表示する。`n` キーで編集モードへ入り、`bubbles/textarea` で複数行を編集する。`Ctrl+S` で保存、`Esc` でキャンセルする。保存は `store.Upsert` 経由（お気に入りトグルと同様に非同期）で行う。

**ノートの保持。** 解析の再生成・キャッシュ更新（2.9 / 2.10）でもノートは保持される。`CloneEntry` が `Note` を引き継ぎ、新規解析時の初期値は空文字とする。

**表示とセキュリティ。** ノートはユーザー自身の入力だが、表示前に制御文字を除去する（既存の `safeText` と同様）。

**CLI。** ノートの編集・表示はTUIで行い、CLI への追加は将来拡張とする（`analyze` は参照用途のまま）。
...
**並行アクセスの整合性。** TUI（`run`）と CLI（`analyze`）は同じ `data.json` を共有する。複数プロセスが同時に書き込んでも `lost update`（相手の追加分が消える）を起こさない。`data.json` 自体のrenameでinodeが入れ替わるため、ロックは一時ファイル経由のrenameでは消えない**別ファイル `data.json.lock`** に対して取得する。`Upsert` / `Delete` の read-modify-write（Load → マージ → Save）は排他ロックの内側で行い、逐次化する。ロックは短時間ブロックし、タイムアウト時は安全なエラーを返す。クラッシュ時もカーネルが自動解放するため、stale ロックは残らない。削除は TUI がメモリスナップショットで `Save` するのではなく、`store.Delete(fullName)` としてストア層のロック内で行う。
...
並行アクセス整合性（3章 非機能要件）。実装はTDD（テストリスト → 失敗するテスト → 実装 → リファクタ）で進め、`internal/store` に `data.json.lock` に対するファイルロックを導入し、`Upsert` と `Delete` の read-modify-write をロック内に閉じ込める。TUIの削除は `store.Delete(fullName)` へ移行する。

---

## 10. 今後の拡張余地

入力を `owner/repo` 文字列に抽象化済みのため、入力元を一覧やフォームに広げても同じ解析パイプラインへ流せる（疑似Trending一覧は 2.12、学習ノートは 2.13 で仕様確定済み）。その他、全文検索、Markdown ノートのエクスポート、ローカル LLM 対応、キーワードからの関連リポジトリ推薦などが考えられる。`analyze` コマンドは複数引数・stdin からの一括解析へ拡張でき、`refreshInterval`（2.10）やコード解析の対象ファイル（2.9）は設定化の余地がある。
