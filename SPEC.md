...
最終更新: 2026-08-10
バージョン: 0.1.0 (開発中)
ステータス: コア・TUI・CLI・CLIプレゼンテーション・OS資格情報ストア移行・コード解析・キャッシュ鮮度管理・analyzeコマンド・疑似Trending一覧実装完了 / 学習ノートは仕様確定・未実装 / OS別手動スモークテスト未完了
...
2.13 学習ノート。実装はTDD（テストリスト → 失敗するテスト → 実装 → リファクタ）で進め、まず `Entry.Note` と保存フロー（TUIの非同期 `Upsert`）を実装し、次に詳細画面の表示・編集UI（`textarea`）を積み上げる。
